#!/usr/bin/env python3
"""
Privileged Information MTL Inference Pipeline (V2)

Performs THz-only inference using the trained privileged information model.
Demonstrates that the model can work without visible images after training.

Features:
1. Face verification with triplet metrics
2. Head posture classification accuracy
3. Image reconstruction quality
4. THz-only operation (no visible images needed)
"""
...
from mtl.unified_data_loader import parse_filename, get_posture_number
...
class TripletsTestDataset(Dataset):
    """Test dataset for triplets inference (THz-only)"""

    def __init__(self, csv_path, transform=None, target_transform=None, input_size=224,
                 data_root="/home/noam/data_fixed/faceData_cropped_64x64"):
...
def create_hstack_visualization(images, titles, save_path):
...
def calculate_verification_metrics(anchor_emb, positive_emb, negative_emb, margin=0.2):
    """Calculate verification metrics using euclidean distance"""
...
def calculate_pixel_metrics(pred, target):
    """Calculate pixel-level metrics for reconstruction"""
...
def run_thz_only_inference(checkpoint_path, test_csv_path, output_dir, input_size=64, max_samples=500):
    """
    Run THz-only inference using the trained privileged information model.
    Demonstrates that visible images are NOT needed for inference.
    """

    # Create output directories
    timestamp = time.strftime('%Y%m%d-%H%M%S')
    base_output_dir = os.path.join(output_dir, f'privileged_thz_only_{input_size}_{timestamp}')
    ...
    # Build the privileged distillation model
    model = PrivilegedDistillationModel(
        num_classes=5,
        embed_dim=64,  # Updated to match SupCon training config
    ...
    print(f"Starting THz-only inference on {min(len(test_loader), max_samples)} samples...")
    print("NOTE: Visible images are NOT used during inference (privileged information framework)")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            # --- THz-Only Forward Pass ---
            # Pass visible_unconcealed=None to use THz-only (student) path
            outputs = model(
                batch['anchor'],
                batch['positive'],
                batch['negative'],
                batch['posture_class'],
                visible_unconcealed=None  # THz-only inference!
            )
            ...
            recon_output = outputs['reconstructed_image']
            ...
            for i in range(batch_size):
                ...
                recon_output = outputs['reconstructed_image'][i:i+1]
                ...
                # Store identity and posture for per-identity/posture analysis
                identity = batch['identity'][i] if isinstance(batch['identity'], list) else str(batch['identity'][i].item())
                posture_class_val = batch['posture_class'][i].item() if isinstance(batch['posture_class'], torch.Tensor) else batch['posture_class'][i]
            ...
            if (batch_idx + 1) % 10 == 0:
                print(f"Processed {total_samples_processed} samples ({batch_idx + 1} batches)...")

    print(f"\nProcessed {len(all_metrics)} samples using THz-only inference")
    ...
    with open(results_txt_path, 'w') as f:
        f.write("Privileged Information MTL Inference Results (THz-Only)\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"Inference Mode: THz-Only (No Visible Images)\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='Run privileged information MTL inference (THz-only)')
    parser.add_argument('--checkpoint', default='/home/noam/PycharmProjects/thz/runs/privileged_mtl_bicubic_64_20251122_000627/checkpoints/best.pth')
    parser.add_argument('--test_csv', default='datafiles_fixed_29_08_sterile/faceData_cropped_64x64/dBm/test_dataset_visible.csv',
                       help='Path to test CSV file')
    parser.add_argument('--output_dir', default='test_runs', help='Output directory for results')
